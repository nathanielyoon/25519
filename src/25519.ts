...
const a_k = (secret_key: Uint8Array) => a_p(sha512(secret_key.subarray(0, 32)));
...
export const sign = (secret_key: Uint8Array, message: Uint8Array) => {
  ...
  const c = sha512(secret_key.subarray(0, 32));
  b.set(c.subarray(32)), b.set(message, 32);
  const d = s(b.subarray(0, a + 32)), e = a_p(c);
  b.set(e_a(wnaf(d))), b.set(e_a(wnaf(e)), 32), b.set(message, 64);
  return c.set(b.subarray(0, 32)), c.set(i_a(p(d + p(s(b) * e, N), N)), 32), c;
};
export const verify = (
...
export const x25519 = (secret_key: Uint8Array, public_key: Uint8Array) => {
  const a = a_n(public_key);
  return i_a(ladder(a_k(secret_key), p((1n + a) * v(1n - a)) & F));
};
